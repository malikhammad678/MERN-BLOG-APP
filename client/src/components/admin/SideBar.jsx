...
const SideBar = () => {
  return (
    <div className="flex flex-col border-r border-gray-200 min-h-full pt-6">
      ...
      <NavLink to={`/admin/add-blog`} className={({ isActive }) =>  `flex items-center gap-3 py-3.5 md:px-9 md:min-w-64 cursor-pointer ${ isActive && "bg-primary/10 border-r-4 border-primary"}`}>
        <img src={assets.add_icon} className="min-w-4 w-5" alt="" />
        <p className="hidden md:inline-block">Add Blog</p>
      </NavLink>
      <NavLink  to={`/admin/list-blog`} className={({ isActive }) =>  `flex items-center gap-3 py-3.5 md:px-9 md:min-w-64 cursor-pointer ${ isActive && "bg-primary/10 border-r-4 border-primary"}`}>
